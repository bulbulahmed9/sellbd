...
exports.register = async (req, res) => {
    try {
        ...
        if (user && !user.password) return res.status(401).json({ message: 'You logged in via gmail before, Please continue with gmail' });
...
exports.login = async (req, res) => {
    try {
        ...
        if (!user.password) return res.status(401).json({ msg: 'You logged in via gmail before, Please continue with gmail' });
...
async function sendVerificationEmail(user, req, res) {
    try {
        ...
        let to = user.email;
        let from = process.env.fromEmail;
